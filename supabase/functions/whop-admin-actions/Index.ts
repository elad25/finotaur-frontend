...
// WHOP API HELPERS
// ============================================
async function whopRequest(endpoint, method = "GET", body) {
  try {
    const options = {
      method,
      headers: {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: `Whop API error: ${response.status} - ${errorText}`
      };
    }
...
// ============================================
async function listMemberships(supabase, product) {
  ...
  if (product === "war_zone") {
    ...
    productIds = [
      PRODUCT_IDS.top_secret
    ];
  }
  // Fetch from Whop API for each product
  const allMemberships = [];
  for (const productId of productIds){
    const result = await whopRequest(`/memberships?product_id=${productId}&per=100`);
  ...
  for (const membership of allMemberships){
    ...
    const { data: profile } = await supabase.from("profiles").select(`
      ...
      `).or(`newsletter_whop_membership_id.eq.${membership.id},top_secret_whop_membership_id.eq.${membership.id},email.ilike.${membership.user?.email || ''}`).maybeSingle();
    if (profile) {
    ...
    const { data: payments } = await supabase.from("whop_webhook_log").select("payload").eq("whop_membership_id", membership.id).eq("event_type", "payment.succeeded");
...
async function getMembership(membershipId) {
  ...
  return await whopRequest(`/memberships/${membershipId}`);
}
// ============================================
// CANCEL MEMBERSHIP
// ============================================
async function cancelMembership(supabase, membershipId, mode = "at_period_end", reason) {
  console.log(`❌ Cancelling membership ${membershipId} (mode: ${mode})...`);
  const result = await whopRequest(`/memberships/${membershipId}/cancel`, "POST", {
    mode
  });
  if (!result.success) {
    return {
      success: false,
      message: result.error || "Failed to cancel"
    };
  }
  // Update local DB
  const { data: profile } = await supabase.from("profiles").select("id, newsletter_whop_membership_id, top_secret_whop_membership_id").or(`newsletter_whop_membership_id.eq.${membershipId},top_secret_whop_membership_id.eq.${membershipId}`).maybeSingle();
  if (profile) {
  ...
  return {
    success: true,
    message: mode === "immediate" ? "Subscription cancelled immediately" : "Subscription will cancel at period end"
  };
}
// ============================================
// RESUME MEMBERSHIP
// ============================================
async function resumeMembership(supabase, membershipId) {
  console.log(`▶️ Resuming membership ${membershipId}...`);
  const result = await whopRequest(`/memberships/${membershipId}`, "POST", {
    cancel_at_period_end: false
  });
  if (!result.success) {
    return {
      success: false,
      message: result.error || "Failed to resume"
    };
  }
  // Update local DB
  const { data: profile } = await supabase.from("profiles").select("id, newsletter_whop_membership_id, top_secret_whop_membership_id").or(`newsletter_whop_membership_id.eq.${membershipId},top_secret_whop_membership_id.eq.${membershipId}`).maybeSingle();
...
// ============================================
async function extendMembership(supabase, membershipId, days) {
  ...
  const currentEnd = membership.renewal_period_end ? new Date(membership.renewal_period_end * 1000) : new Date();
  ...
  if (!result.success) {
    ...
    const altResult = await whopRequest(`/memberships/${membershipId}/add_free_days`, "POST", {
      days
    });
    if (!altResult.success) {
      return {
        success: false,
        message: result.error || "Failed to extend membership"
      };
    }
  }
  // Update local DB
...
// BAN USER
// ============================================
async function banUser(supabase, membershipId, reason) {
  ...
  return {
    success: true,
    message: "User banned and subscription cancelled"
  };
}
// ============================================
// UNBAN USER
// ============================================
async function unbanUser(supabase, userId) {
...
// ============================================
async function generateTransferLink(membershipId) {
  console.log(`🔗 Generating transfer link for ${membershipId}...`);
  const result = await whopRequest(`/memberships/${membershipId}/transfer_link`, "POST");
  if (!result.success || !result.data?.transfer_url) {
    return {
      success: false,
      message: result.error || "Failed to generate transfer link"
    };
  }
  return {
    success: true,
    transfer_url: result.data.transfer_url
  };
}
// ============================================
// SYNC LOCAL DB WITH WHOP
// ============================================
async function syncWithWhop(supabase, product) {
  ...
  for (const membership of listResult.data){
    ...
    if (product === "war_zone") {
      ...
      updateData.newsletter_status = membership.status === "active" ? "active" : membership.status === "trialing" ? "trial" : membership.status;
      ...
      updateData.top_secret_status = membership.status === "active" ? "active" : membership.status === "trialing" ? "trial" : membership.status;
    ...
    const { error } = await supabase.from("profiles").update(updateData).eq("id", membership.local_user_id);
    if (!error) {
      syncedCount++;
    }
  }
...
// VERIFY ADMIN
// ============================================
async function verifyAdmin(supabase, authHeader) {
    ...
    switch(action){
      ...
      case "extend":
        if (!membership_id || !days) {
          return new Response(JSON.stringify({
            error: "Membership ID and days required"
          }), {
            status: 400,
            headers: {
              ...corsHeaders,
