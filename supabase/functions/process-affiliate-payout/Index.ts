...
async function createPayPalPayout(accessToken, payoutId, recipientEmail, amount, note) {
  const mode = Deno.env.get('PAYPAL_MODE') || 'sandbox';
  const baseUrl = mode === 'live' ? PAYPAL_LIVE_URL : PAYPAL_SANDBOX_URL;
...
async function getPayoutStatus(accessToken, batchId) {
  const mode = Deno.env.get('PAYPAL_MODE') || 'sandbox';
  const baseUrl = mode === 'live' ? PAYPAL_LIVE_URL : PAYPAL_SANDBOX_URL;
...
serve(async (req)=>{
  ...
  try {
    ...
    if (action === 'request') {
      if (!affiliateId || !amount) {
        return new Response(JSON.stringify({
          success: false,
          error: 'Missing affiliateId or amount'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      const { data: affiliate, error: affiliateError } = await supabase.from('affiliates').select('id, user_id, paypal_email, total_earnings_usd, total_paid_usd').eq('id', affiliateId).single();
      ...
      if (!affiliate.paypal_email) {
        return new Response(JSON.stringify({
          success: false,
          error: 'PayPal email not set. Please add your PayPal email first.'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      const { data: availableCommissions } = await supabase.from('affiliate_commissions').select('id, commission_amount_usd').eq('affiliate_id', affiliateId).eq('status', 'confirmed').is('payout_id', null).order('created_at', {
        ascending: true
      });
      const availableBalance = availableCommissions?.reduce((sum, c)=>sum + Number(c.commission_amount_usd), 0) || 0;
      ...
      const { data: existingPayouts } = await supabase.from('affiliate_payouts').select('id, total_amount_usd, status').eq('affiliate_id', affiliateId).in('status', [
        'pending',
        'processing'
      ]);
      ...
      ;
      const { data: newPayout, error: insertError } = await supabase.from('affiliate_payouts').insert({
      ...
      if (commissionIds.length > 0) {
        const { error: lockError } = await supabase.from('affiliate_commissions').update({
          payout_id: newPayout.id,
          updated_at: new Date().toISOString()
        }).in('id', commissionIds);
        if (lockError) {
          // Rollback: delete the payout if we couldn't lock commissions
          await supabase.from('affiliate_payouts').delete().eq('id', newPayout.id);
    ...
    if (action === 'process') {
      ...
      // Get payout details
      const { data: payout, error: payoutError } = await supabase.from('affiliate_payouts').select(`
          *,
      ...
      if (!paypalEmail) {
        return new Response(JSON.stringify({
          success: false,
          error: 'No PayPal email found for this payout'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      const paypalResult = await createPayPalPayout(accessToken, payoutId, paypalEmail, payout.total_amount_usd, `Finotaur Affiliate Commission - ${payout.affiliate?.referral_code || 'N/A'}`);
    ...
    if (action === 'check_status') {
      ...
      // Get payout details
      const { data: payout, error: payoutError } = await supabase.from('affiliate_payouts').select('*').eq('id', payoutId).single();
      if (payoutError || !payout) {
        return new Response(JSON.stringify({
          success: false,
          error: 'Payout not found'
      ...
      if (!payout.paypal_batch_id) {
        return new Response(JSON.stringify({
          success: false,
          error: 'No PayPal batch ID for this payout'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      if (newStatus !== payout.status) {
        ...
        if (newStatus === 'completed') {
          ...
          await supabase.from('affiliate_commissions').update({
            ...
            payout_id: payoutId,
            updated_at: new Date().toISOString()
          }).eq('affiliate_id', payout.affiliate_id).eq('status', 'confirmed').is('payout_id', null).lte('commission_amount_usd', payout.total_amount_usd);
      ...
      await supabase.from('affiliate_payouts').update(updateData).eq('id', payoutId);
      return new Response(JSON.stringify({
        success: true,
        status: newStatus,
    ...
    if (action === 'batch_process') {
      ...
      const { data: pendingPayouts, error } = await supabase.from('affiliate_payouts').select(`
          *,
      ...
      for (const payout of pendingPayouts){
        ...
        const paypalResult = await createPayPalPayout(accessToken, payout.id, paypalEmail, payout.total_amount_usd, `Finotaur Affiliate Commission - ${payout.affiliate?.referral_code || 'N/A'}`);
        if (paypalResult.success) {
          await supabase.from('affiliate_payouts').update({
            status: 'processing',
            paypal_batch_id: paypalResult.batchId,
    ...
    if (action === 'cancel') {
      ...
      // Get payout details
      const { data: payout, error: payoutError } = await supabase.from('affiliate_payouts').select('id, status, affiliate_id').eq('id', payoutId).single();
      if (payoutError || !payout) {
        return new Response(JSON.stringify({
          success: false,
          error: 'Payout not found'
