-- =====================================================
-- 2026-05-20  Fix type mismatch in get_published_reports_for_user
-- =====================================================
-- Follow-up to 2026-05-20_exclude_daily_from_top_secret_dashboard.sql.
--
-- The RPC declared `visibility text` in its RETURNS TABLE() signature,
-- but `published_reports.visibility` is `character varying(10)`. Postgres
-- rejected every call with:
--   ERROR 42804: structure of query does not match function result type
--   DETAIL: Returned type character varying(10) does not match expected
--           type text in column 24.
--
-- The frontend caught and swallowed the error silently (Dashboard simply
-- rendered an empty state) — which is why TOP SECRET Dashboard appeared
-- broken for all users since the function was first created.
--
-- Fix: cast `pr.visibility::text` in the SELECT projection. Function
-- signature unchanged.
-- =====================================================
...
BEGIN
    ...
    SELECT
        ...
        pr.created_at,
        pr.updated_at,
        pr.visibility::text AS visibility,   -- 🔥 FIX: cast varchar(10) -> text
        pr.market_regime,
        pr.pmi_value,
        pr.target_group,
