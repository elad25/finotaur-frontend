// src/components/copyTrading/AddConnectionModal.tsx
...
// Only 'tradovate' has an active form; others are coming-soon placeholders.
...
// Tradovate is the only broker with a live credential form today.
...
export const AddConnectionModal = memo(function AddConnectionModal({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // connect() fires its own toast on success/failure — we only surface
      // a local error banner if the result indicates failure.
  ...
  // Resolve which brokers to show; only show those present in BROKER_CONFIGS.
  const visibleBrokers = GRID_BROKERS.filter(
    (b) => b in BROKER_CONFIGS
  );
      ...
      >
        ...
        <div className="px-ds-5 pt-ds-5 pb-ds-3">
          ...
          <p className="text-xs text-ink-secondary mt-1">
            A connection links FINOTAUR to a set of trading accounts under the same broker login.
          </p>
          <div className="text-[10px] text-ink-tertiary mt-ds-2 uppercase tracking-wider">
            {usedCount}/{CONNECTION_LIMIT} connections used
          </div>
        </div>
        ...
        <div className="px-ds-5 pb-ds-3 space-y-ds-3">
          ...
          {/* Per-broker credential form — Tradovate only */}
          ...
          {error && (
            <div className="rounded-md bg-num-negative/10 border border-num-negative/30 px-ds-3 py-ds-2">
              <p className="text-xs text-num-negative">{error}</p>
            </div>
          )}
        </div>
