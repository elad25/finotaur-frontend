...
export default function AddBrokerPopup({ open, onOpenChange }: Props) {
  ...
  // Tradovate + NinjaTrader use the legacy username/password flow (tradovate-auth
  // mode=login) because Tradovate's OAuth trading_read scope returns []
  // from /v1/account/list — accounts/portfolios cannot be discovered via OAuth.
  // OAuth backend (oauth-callback v9, oauth-refresh, etc.) stays deployed for
  // future re-enablement; only the modal UX path is reverted to username/password.
  const isOAuth = config.features.oauth && selectedBroker !== 'tradovate' && selectedBroker !== 'ninja_trader';
            ...
            {usesTradovateAuth && (
              <div className="overflow-hidden rounded-[12px] border border-white/10 bg-[#101010]/70">
                <CredentialInput
                  ...
                  placeholder={isNinjaTrader ? 'NinjaTrader username' : 'APEX_19585, TOPSTEP_12345, your@email.com'}
