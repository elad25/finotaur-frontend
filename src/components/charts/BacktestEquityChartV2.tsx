// ================================================
// BACKTEST EQUITY CHART V2 — cloned from EquityChart.tsx, blue accent
// Gold (#C9A646 / #E8C766 / #B89B45) replaced with blue (#7AB6F4 / #5C9BDF / #4A8AD4)
// Do NOT modify EquityChart.tsx — this file is Backtest-only.
// ================================================
...
// Blue accent palette (replaces gold)
const BLUE = '#7AB6F4';
const BLUE_MID = '#5C9BDF';
const BLUE_DIM = 'rgba(122,182,244,0.2)';
...
interface BacktestEquityChartV2Props {
  data: EquityData[];
  trades?: EquityTrade[];
}

const BacktestEquityChartV2 = React.memo(({ data, trades = [] }: BacktestEquityChartV2Props) => {
      ...
      <div className="mb-3 flex items-start justify-between">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-md border border-white/[0.08] px-3 py-1.5 text-[11px] font-medium text-white/78 transition-colors hover:border-[#7AB6F4]/28 hover:text-[#7AB6F4]"
            aria-haspopup="menu"
          ...
          {selectorOpen && (
            <div className="absolute right-0 top-8 z-20 w-32 overflow-hidden rounded-[10px] border border-[#7AB6F4]/18 bg-[#101010] p-1 shadow-[0_16px_34px_rgba(0,0,0,0.42)]">
              {([
                ['daily', 'Daily'],
                ['trades', 'Trades'],
      ...
      <div style={{ width: "100%", height: 210 }}>
        <ResponsiveContainer>
          <AreaChart data={optimizedData}>
            <defs>
              <linearGradient id="btEqGradient" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor={CHART_COLORS.profit} stopOpacity={0.3}/>
                <stop offset="100%" stopColor={BLUE} stopOpacity={0.3}/>
              </linearGradient>
              <linearGradient id="btLineGradient" x1="0" y1="0" x2="1" y2="0">
            ...
            <Area
              type="monotone"
              dataKey="equity"
              stroke="url(#btLineGradient)"
              fill="url(#btEqGradient)"
              strokeWidth={2.5}
              filter="url(#btGlow)"
              isAnimationActive={false}
              dot={false}
              activeDot={{ r: 5, fill: BLUE_MID, stroke: CHART_COLORS.backgroundDark, strokeWidth: 2 }}
...
BacktestEquityChartV2.displayName = 'BacktestEquityChartV2';

export default BacktestEquityChartV2;
