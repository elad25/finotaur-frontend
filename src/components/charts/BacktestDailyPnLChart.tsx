// ================================================
// BACKTEST DAILY P&L CHART - PRODUCTION READY
// File: src/components/charts/BacktestDailyPnLChart.tsx
// ✅ Adapted for backtest equity curve data
// ✅ Calculates daily P&L from equity changes
...
interface BacktestDailyPnLChartProps {
  data: BacktestEquityPoint[];
}

const BacktestDailyPnLChart = React.memo(({ data }: BacktestDailyPnLChartProps) => {
  ...
  const dailyPnLData = useMemo(() => {
    ...
    for (let i = 1; i < data.length; i++) {
      const currentEquity = data[i].value;
      const previousEquity = data[i - 1].value;
      const dailyPnL = currentEquity - previousEquity;
      
      result.push({
        date: dayjs(data[i].date).format('MMM DD'),
  ...
  const optimizedData = useMemo(() => {
    if (!dailyPnLData || dailyPnLData.length === 0) return [];
    
    // If more than 100 bars, downsample
    if (dailyPnLData.length > 100) {
      const step = Math.ceil(dailyPnLData.length / 100);
      return dailyPnLData.filter((_, index) => index % step === 0);
    }
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-[#F4F4F4] font-semibold text-lg tracking-tight">
              BACKTEST: Net Daily P&L
            </h3>
        ...
        <div className="flex flex-col items-center justify-center h-80">
          ...
          <div className="text-[#F4F4F4] text-base font-medium mb-2">No Backtest Data</div>
          <div className="text-[#A0A0A0] text-sm font-light text-center max-w-xs">
            Run a backtest to see daily P&L performance
          </div>
        </div>
      </div>
    );
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#A0A0A0] text-sm font-light">
            Daily performance tracking over backtest period (one bar per day)
          </p>
        </div>
      ...
      <div style={{ width: "100%", height: 340 }}>
        <ResponsiveContainer>
          ...
          >
            <defs>
              <linearGradient id="backtestProfitGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <linearGradient id="backtestLossGradient" x1="0" y1="1" x2="0" y2="0">
            ...
            <Bar 
              ...
              radius={[4, 4, 0, 0]}
              maxBarSize={14}
              filter="url(#backtestBarShadow)"
              isAnimationActive={false} // ✅ Disable animation for performance
              shape={(props: any) => {
                const { x, y, width, height, payload } = props;
                const isPositive = payload.pnl >= 0;
                const fillColor = isPositive ? "url(#backtestProfitGradient)" : "url(#backtestLossGradient)";
...
BacktestDailyPnLChart.displayName = 'BacktestDailyPnLChart';
