...
const RecentSignupToast = memo(function RecentSignupToast() {
  ...
  useEffect(() => {
    ...
    load();
    const refreshId = window.setInterval(load, 3 * 60 * 1000);
  ...
  return (
    <section className="relative z-20 -mt-10 px-4 pb-10 md:-mt-12">
      <motion.div
        key={`${activeSignup.firstName}-${activeSignup.signedUpAt}`}
        initial={{ opacity: 0, y: 16, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.45, ease: 'easeOut' }}
        className="mx-auto flex w-full max-w-md items-center gap-4 rounded-xl border border-[#C9A646]/25 bg-[#0f0d09]/95 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.45)] backdrop-blur-md"
      >
