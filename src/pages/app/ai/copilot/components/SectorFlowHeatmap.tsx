// 25 industry tiles (Semiconductors, Cybersecurity, Solar, etc.) grouped by
// parent sector for visual scanning only — the industry name is the headline,
// the parent grouping is secondary chrome.
...
function Tile({ etf }: { etf: ScoredEtf }) {
  ...
  return (
    <div
      className={`flex flex-col justify-between rounded-[12px] border border-border-ds-subtle ${tint} p-ds-3 w-[150px] min-h-[100px] transition-transform duration-150 hover:scale-[1.04] ${glow}`}
      title={`${etf.subSector} — composite ${etf.composite.toFixed(1)}`}
    >
      <div className="text-sm text-ink-primary leading-tight">{etf.subSector}</div>
      <div className="flex flex-col gap-1">
        <Change value={etf.priceChangePct} format="percent" />
        <div className="text-[10px] font-mono text-ink-tertiary tracking-wide">{etf.etf}</div>
      </div>
    </div>
  );
}
  ...
  if (loading && ranked.length === 0) {
    return (
      <div className="rounded-[12px] border border-border-ds-subtle bg-surface-1 p-ds-5">
        <div className="text-xs uppercase tracking-[0.18em] text-ink-secondary mb-ds-3">25 Industries Tracked</div>
  ...
  return (
    <div className="rounded-[12px] border border-border-ds-subtle bg-surface-1 p-ds-5">
      <div className="flex items-baseline justify-between mb-ds-4">
        <div className="text-xs uppercase tracking-[0.18em] text-gold-primary">25 Industries Tracked</div>
