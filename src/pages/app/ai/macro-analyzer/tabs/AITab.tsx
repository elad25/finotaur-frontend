// tabs/AITab.tsx
// =====================================================
// 🤖 AI MACRO INTELLIGENCE TAB
// AI-powered analysis combining all data sources into
// actionable intelligence for traders & investors.
//
// EFFICIENCY FOR 10K USERS:
// ✅ ALL AI calls go through backend (shared cache)
// ✅ GET /ai-analysis = zero cost (reads cache only)
// ✅ POST /ai-generate/:section = 1 AI call for ALL users
// ✅ No direct Anthropic/OpenAI calls from client
// ✅ Signal Dashboard = pure data computation (no AI)
// ✅ Key Movers = pure data computation (no AI)
// ✅ LazySection = only render what's visible
//
// COST: 10K users × 3 sections = 3 AI calls per hour (not 30K)
// =====================================================
...
// =====================================================
// QUICK SIGNALS (non-AI, computed from data — zero cost)
// =====================================================
  ...
  const signals = useMemo(() => {
    ...
    if (fed?.yieldCurve != null) {
      result.push({
        ...
        detail: fed.yieldCurve > 0.5 ? 'Steep positive — expansion'
    ...
    <Card>
      <div className="p-6">
        <SectionHeader
          ...
          subtitle="Real-time macro signals — no AI, pure data"
          action={
            <div className="flex items-center gap-2">
        ...
        <div className="mt-4 p-3 rounded-lg bg-white/[0.02]">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-[#8B8B8B]">
              {bullishCount} bullish · {signals.length - bullishCount - bearishCount} neutral · {bearishCount} bearish
            </span>
          </div>
...
// =====================================================
// KEY MOVERS (top changing indicators — no AI cost)
// =====================================================
              ...
              <div key={ind.id} className="p-4 rounded-xl bg-white/[0.03] hover:bg-white/[0.05] transition-colors">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge variant={ind.trend === 'improving' ? 'success' : ind.trend === 'declining' ? 'danger' : 'warning'}>
                    {ind.trend === 'improving' ? '↑' : ind.trend === 'declining' ? '↓' : '→'}
                  </Badge>
                </div>
      ...
      <Card highlight>
        <div className="relative p-6">
          ...
          <p className="text-[10px] text-[#6B6B6B] flex items-center gap-2 mt-2">
            ...
            AI responses cached on server for all users · Data refreshes automatically
          </p>
        </div>
      </Card>
