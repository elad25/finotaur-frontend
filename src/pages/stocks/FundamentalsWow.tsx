...
export default function FundamentalsWow(){
  ...
  async function loadAll(s:string,p:'quarterly'|'annual'){
    ...
    try{
      const [snap, ins, pr] = await Promise.all([
        ...
        fetch(`/api/fundamentals/peers?symbol=${encodeURIComponent(s)}`).then(r=>r.json()),
      ]);
  ...
  const ratiosRows = useMemo(()=>{
    ...
    x.push({category:'Profitability', metric:'ROE', value:pct(snapshot.roe), interpretation:'Profitability on equity'});
    x.push({category:'Profitability', metric:'ROA', value:pct(snapshot.roa), interpretation:'Profitability on assets'});
