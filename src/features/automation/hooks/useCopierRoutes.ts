...
//
// The upsert RPC now uses account-based identity (source_account_id /
// destination_account_id) instead of the legacy connection-based approach.
// ─────────────────────────────────────────────────────────────────────────────
...
export function useCopierRoutes() {
   ...
   * @param sourceAccount    The source (copy FROM) journal account.
   ...
   * @param targets          Array of destination account targets.
   */
  const upsertRoute = useCallback(
    async (params: {
      ...
      const { data, error: e } = await supabase.rpc('automation_upsert_copier_route', {
        ...
        p_source_account_id:   params.sourceAccount.account_id,
        p_source_account_name: params.sourceAccount.account_name,
        p_source_broker:       params.sourceAccount.broker,
        p_source_environment:  params.sourceAccount.environment,
