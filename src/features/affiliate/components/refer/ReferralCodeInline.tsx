...
//
// Renders as a business-card teaser (brand asset image) that expands on
// click to reveal the code/link + provision action — state machine below
// is unchanged, only the rendered markup for no-code/provisioning/error/
// ready collapses into one shared expandable card.
//
...
export function ReferralCodeInline() {
  ...
  // Shared "business card" teaser — collapsed shows just the brand image with
  // a "tap to reveal" hint; expanded reveals the bonus copy plus whichever
  // action fits the current state (provision button, or code/link + copy).
  const code = state.kind === 'ready' ? (state.row.coupon_code as string) : null;
  const link = code ? buildReferralLink(code) : null;

  return (
    <div className="overflow-hidden rounded-xl border border-[#C9A646]/25 bg-zinc-900/40 shadow-[0_18px_50px_-24px_rgba(0,0,0,0.9)]">
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        aria-expanded={expanded}
        className="group block w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#C9A646]"
      >
      ...
      {expanded && (
        <div className="flex flex-col gap-3 border-t border-zinc-700/30 p-5">
