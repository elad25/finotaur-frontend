...
async function getTickerMap() {
  ...
  const map = {};
  for (const row of j) map[String(row.ticker||"").toUpperCase()] = row;
  _tickerMap = map; return _tickerMap;
}
...
function findDocUrlFromSubmissions(subs, targetForm, targetFilingDate){
  try{
    ...
    for (let i=0;i<forms.length;i++){
      if (String(forms[i]).toUpperCase() === String(targetForm||"").toUpperCase()
        ...
        const url = buildDocUrl(subs.cik, accessions[i], docs[i]);
        if (url) return url;
      }
    }
  }catch {}
...
module.exports = { getCikForSymbol, buildDocUrl, getCompanySubmissions, findDocUrlFromSubmissions };
