...
import { calculateActualR, getAssetMultiplier } from '@/utils/tradeCalculations';
 ...
 * 🔥 FIXED: Use actual_r instead of rr
 */
export function calculateAllStats(trades: any[]): StrategyStats {
  ...
  for (const trade of trades) {
    // 🔥 FIXED: Use actual_r from top level OR metrics OR calculate it
    let r = trade.actual_r ?? trade.metrics?.actual_r;
    
    // If not stored, calculate it
    if (r === undefined && trade.exit_price) {
      r = calculateActualR(
        trade.entry_price,
        trade.stop_price,
        trade.exit_price,
    ...
    // If still undefined, skip this trade
