 ...
 * 🔥 FIXED: Now respects userId parameter for admin access
 */
export async function getTrades(userId?: string) {
  try {
    // 🔥 FIX: Use provided userId if available, otherwise get effective user
    const targetUserId = userId || await getEffectiveUserId();
    
    if (!targetUserId) {
      return { ok: false, message: "Not authenticated", data: [] };
    }

    console.log('📊 getTrades:', { targetUserId, wasProvided: !!userId });
 ...
 * 🔥 FIXED: Correct compressImage parameters
 */
export async function uploadScreenshot(file: File): Promise<string | null> {
  try {
    ...
    try {
      ...
      // 🔥 FIX: Parameters are (file, maxSizeMB, maxWidthOrHeight, quality)
