...
import { getTickerMap, padCik, getSubmissionsByCIK, buildFilingUrl } from '../services/secCore';
...
router.get('/filings', async (req, res) => {
  try {
    ...
    const N = Math.min(recent.form?.length || 0, recent.filingDate?.length || 0, recent.accessionNumber?.length || 0, recent.primaryDocument?.length || 0);
    const filings = [] as any[];
    for (let i = 0; i < N; i++) {
      filings.push({
        form: recent.form[i], accessionNumber: recent.accessionNumber[i], filingDate: recent.filingDate[i],
        reportDate: recent.reportDate?.[i], primaryDocument: recent.primaryDocument[i],
        filingUrl: buildFilingUrl(cik, recent.accessionNumber[i], recent.primaryDocument[i]),
...
router.get('/filings/latest', async (req, res) => {
  try {
    ...
    const sub = await getSubmissionsByCIK(cik);
    const recent = sub?.filings?.recent || {};
    const N = Math.min(recent.form?.length || 0, recent.filingDate?.length || 0, recent.reportDate?.length || 0, recent.accessionNumber?.length || 0, recent.primaryDocument?.length || 0);
    const rows = [];
    for (let i = 0; i < N; i++) {
      rows.push({
        form: recent.form?.[i],
